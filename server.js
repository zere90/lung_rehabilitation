...
app.post('/api/test/submit', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    // Если тест пройден, создаем сертификат
    if (passed) {
      const user = await User.findById(req.session.userId);
      const certNumber = `CERT-${Date.now()}-${user.username.substring(0, 3).toUpperCase()}`;
...
// ============ ПОЛУЧИТЬ СЕРТИФИКАТ ============
app.get('/api/certificate', isAuthenticated, async (req, res) => {
  try {
